...
class UniqueIDSingleton(type):
    _instances = {}

    def __call__(cls, json_path, id, *args, **kwargs):
        if id not in cls._instances:
            instance = super().__call__(json_path, id, *args, **kwargs)
    ...
    for x in dir(obj):
        if subString == '' or subString in x.lower():
            try:
                ...
                unreal.log_warning(f"getattr: {x} got exception: {e}")
                if x.startswith("on_"):
                    propertiesNames.append(x)
...
def guess_instance_name(json_file_path, bPrint=True):
    try:
        with open(json_file_path, 'r', encoding='utf-8') as f:
            py_cmd = json.load(f).get("InitPyCmd", "")
            instance_name = next(line[:line.find('=')].strip() for line in py_cmd.split(";") if "%jsonpath" in line.lower())
            if "%" in instance_name:
                # In some case, we don't assign the instance to an explicit variable name
